...
def train_random_forest(X_train, X_test, y_train, y_test, target_columns):
    ...
    for i, target in enumerate(target_columns):
        # Train individual model for each target
...
def generate_future_predictions(models, df_eng, feature_cols, selected_targets, hours=168):
    """Generate future predictions for the next hours for all models"""
    future_predictions = {}
        ...
        try:
            if model_key == 'LSTM':
                ...
                for hour in range(hours):
                    ...
                    for i, target in enumerate(selected_targets):
                        rf_model = model_data[target]
                        pred = rf_model.predict(current_features[feature_cols])[0]
                        future_preds[target].append(pred)
                    
                    # Update features for next prediction (simplified approach)
                    if hour < hours - 1:
                        # Create new row by copying current features
                        new_row = current_features.iloc[-1:].copy()
                        
                        # Update lag features (simplified)
                        for target in selected_targets:
                            # Shift lag features
                            for lag in range(2, 0, -1):
                                if f'{target}_lag_{lag}' in feature_cols:
                                    if lag == 1:
                                        new_row[f'{target}_lag_{lag}'] = future_preds[target][-1]
                                    else:
                                        new_row[f'{target}_lag_{lag}'] = current_features[f'{target}_lag_{lag-1}'].iloc[-1]
                        
                        # Update time features (increment by 1 hour)
                        if 'hour' in feature_cols:
                            new_row['hour'] = (new_row['hour'].iloc[-1] + 1) % 24
                        
                        current_features = pd.concat([current_features, new_row], ignore_index=True).iloc[-1:]
                ...
                for hour in range(hours):
                    # Make prediction for current features
                    for i, target in enumerate(selected_targets):
                        xgb_model = model_data[target]
                        pred = xgb_model.predict(current_features[feature_cols])[0]
                        future_preds[target].append(pred)
...
def create_model_comparison_plot(all_scores, selected_targets):
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('RMSE Comparison (Lower is Better)', 'R² Score Comparison (Higher is Better)'),
    ...
    for i, model in enumerate(models):
        rmse_values = [all_scores[model][target]['rmse'] for target in selected_targets]
        fig.add_trace(
            go.Bar(
                x=selected_targets,
                y=rmse_values,
    ...
    # R² plot
    for i, model in enumerate(models):
        r2_values = [all_scores[model][target]['r2'] for target in selected_targets]
        fig.add_trace(
            go.Bar(
                x=selected_targets,
                y=r2_values,
    ...
    fig.update_xaxes(title_text="Target Variables", row=1, col=1)
    fig.update_xaxes(title_text="Target Variables", row=1, col=2)
...
def main():
    ...
    st.markdown("""
    This app predicts future values of PM2.5, PM10, CO2, CO, Temperature, and Humidity using multiple ML models.
    **Compare LSTM, Random Forest, and XGBoost models** and see their predictions in hourly and weekly plots.
    ...
    available_models = ['LSTM', 'Random Forest', 'XGBoost']
    ...
    training_samples = st.sidebar.slider(
        ...
        step=100,
        help="Select how many recent samples to use for training the models"
    )
    
    # Sequence length selection (only for LSTM)
    ...
    # Epochs selection (only for LSTM)
    if 'LSTM' in selected_models:
    ...
    st.info(f"Using {len(df_eng)} samples for training the models")
    ...
    for model_name in selected_models:
        ...
        try:
            if model_name == 'LSTM':
                with st.spinner(f'Training LSTM with {len(df_eng)} samples...'):
                    # Prepare LSTM data
                    ...
                    all_predictions['LSTM'] = {col: predictions[:, i] for i, col in enumerate(selected_targets)}
                    all_scores['LSTM'] = scores
                
            elif model_name == 'Random Forest':
                with st.spinner(f'Training Random Forest with {len(X_train)} samples...'):
                    models, predictions, scores = train_random_forest(X_train, X_test, y_train, y_test, selected_targets)
                    all_models['Random Forest'] = models
                    all_predictions['Random Forest'] = predictions
                    all_scores['Random Forest'] = scores
                
            elif model_name == 'XGBoost':
                with st.spinner(f'Training XGBoost with {len(X_train)} samples...'):
                    models, predictions, scores = train_xgboost(X_train, X_test, y_train, y_test, selected_targets)
                    all_models['XGBoost'] = models
                    all_predictions['XGBoost'] = predictions
                    all_scores['XGBoost'] = scores
            
            # Display scores for this model
            if model_name in all_scores:
                scores_df = pd.DataFrame(all_scores[model_name]).T
    ...
    if len(all_scores) > 1:
        ...
        comparison_fig = create_model_comparison_plot(all_scores, selected_targets)
        if comparison_fig:
